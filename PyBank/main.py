...
with open('Resources/budget_data.csv', 'r') as csv_file:
    ...
    # Final output being written to output file in analysis folder
    with open("analysis/output.txt", "w") as f:
        f.write("Financial Analysis\n")
        f.write("----------------------------\n")
        f.write(f"Total Months: {total_number_of_months}\n")
        f.write(f"Total: ${net_total_of_profits_over_entire_period}\n")
        f.write(f"Average Change: ${average_change_sum}\n")
        f.write(f"Greatest Increase in Profits: {greatest_increase['Date']} (${greatest_increase['Profit_change']})\n")
        f.write(f"Greatest Decrease in Profits: {greatest_decrease['Date']} (${greatest_decrease['Profit_change']})")
